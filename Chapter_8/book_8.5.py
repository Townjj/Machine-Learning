...
def gini_index_continue(D,a):
    ...
    for item in candidate:
        ...
        for item1 in temp_dict.values():
            p1_num ,p2_num = 0,0
            for index in item1:
                if data[index-1][-1] == '好瓜': p1_num += 1
                if data[index-1][-1] == '坏瓜': p2_num += 1
...
def treegenerate(D,A,d):
    ...
    gini_list = {}  #{属性：gini}
    for a in A:
        max_t,min_gini_index,Da_dis = gini_index_continue(D,a)
...
def bagging(data,result,T):
    ...
    for t in range(T):
        ...
        random_list,train_data = [],[]
        for i in range(16):
            random_num = random.randint(0,16)
            random_list.append(random_num)
            train_data.append(data_copy[random_num])
        tree,predict_true_list = treegenerate(train_data,A=[1,2],d=[['编号','密度','含糖量','好瓜']])
        predict_true_list = [i-1 for i in predict_true_list]
        predict_true_rate = len(predict_true_list) / len(data)
        #print('第',t,'个基学习器','\ntree=',tree,'\npredict true rate =',predict_true_list)
        predict_list[t,:] = [0]*len(data)
        for index in random_list:
            if index not in predict_true_list:
                predict_list[t,index] = result[index]*(-1)     #同一样本在同一基学习器中预测正确多次应累加
            else:predict_list [t,index] = result[index]

        
    print('第',t,'个基学习器','\npredict=\n',predict_list)
    fusion_result = np.sum(predict_list,axis=0)
    fusion_true_num = 0 
    ...
    return fusion_result,fusion_true_rate
...
for T in (3,8,15):
    bagging_result,bagging_rate = bagging(data,result,T)
    print(T,'个集成学习器输出结果为\n',bagging_result,'\n正确率为',bagging_rate,'\n------------')

tree,predict_true_list= treegenerate(tree_train_data,A=[1,2],d=[['编号','密度','含糖量','好瓜']])
print('\n---单个决策树的正确率为---\n',tree,'\n',len(predict_true_list)/len(data),'\n-----------\n')